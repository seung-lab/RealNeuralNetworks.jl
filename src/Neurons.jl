export load_swc, save_swc, get_neuroglancer_precomputed, save_neuron_bin, load_neuron_bin
...
@inline function Neuron(nodeList::Vector{NTuple{4,T}}, 
                         connectivityMatrix::SparseMatrixCSC{Bool, UInt32}) where {T}
    nodeArray = node_list_to_array(nodeList)
    Neuron{T}(nodeArray, connectivityMatrix)
end 
...
##################### getters ###############################
@inline function get_node_array(self::Neuron) self.nodeArray end 

@inline function get_node_list(self::Neuron{T}) where T 
...
@inline function get_connectivity_matrix(self::Neuron) self.connectivityMatrix end
@inline function get_classes(self::Neuron) self.classes end 
@inline function get_radii(self::Neuron) @view self.nodeArray[4, :] end 
function get_node_num(self::Neuron; class::Union{Nothing, UInt8}=nothing)
    if class == nothing 
...
@inline function get_parents(self::Neuron)
    connectivityMatrix = get_connectivity_matrix(self)
    connectivity_matrix_to_parents(connectivityMatrix)
end

@inline function get_root_node_index_list(self::Neuron)
    parents = get_parents(self)
    findall(parents.==zero(UInt32))
end

function get_children_node_index_list(self::Neuron, nodeIdx::Integer)
    connectivityMatrix = get_connectivity_matrix(self)
    childrenNodeIdxes,_ = findnz(connectivityMatrix[:, nodeIdx])
    childrenNodeIdxes
end

function get_parent_node_index(self::Neuron, nodeIdx::Integer)
    connectivityMatrix = get_connectivity_matrix(self)
...
"""
@inline function get_edge_num(self::Neuron) nnz(self.connectivityMatrix) end

function get_terminal_node_id_list(self::Neuron)
...
function get_branching_node_id_list(self::Neuron)
...
#################### Setters ############################################
@inline function set_radii!(self::Neuron, radius::Float32)
    nodeArray = get_node_array(self)
    nodeArray[4, :] .= radius
end

@inline function set_radii!(self::Neuron{T}, radii::Vector{T}) where T
    nodeArray = get_node_array(self)
    nodeArray[4, :] = radii
end

#################### Base functions ######################################
function Base.isequal(self::Neuron{T}, other::Neuron{T}) where T
...
@inline function Base.:(==)(self::Neuron{T}, other::Neuron{T}) where T
    isequal(self, other)
end
...
"""
    Base.iterate(self::Neuron, state::NTuple{2,Int}=(1,1))

depth first search of tree. 
We assume that Neuron is a forest of trees.
The first element of state is tree index.
The second element of state is node index in forrest (not in tree!).
"""
function Base.iterate(self::Neuron, state::NTuple{2,Int}=(1,1); root)
...
"""
    Base.getindex(self::Neuron, idx::Integer)

return a node of this neuron. The node is a view without memory copy.
The vector is [x,y,z,r] 
"""
@inline function Base.getindex(self::Neuron, idx::Integer)
    nodeArray = get_node_array(self)
    @view nodeArray[:, idx]
end

function Base.UnitRange(self::Neuron)
    nodeArray = get_node_array(self)
...
"""
    find_closest_node_id(self::Neuron{T}, point::NTuple{3,T}) where T
...
"""
    get_path_length( self::Neuron )
accumulate all the euclidean distance of edges 
"""
function get_path_length( self::Neuron{T} ) where T
    nodeArray = get_node_array(self)
    connectivityMatrix = get_connectivity_matrix(self)
    childNodeIdxes, parentNodeIdxes, _ = findnz(connectivityMatrix)
...
##################### transformation ##########################
"""
get binary buffer formatted as neuroglancer Neuron.
...
"""
    load_neuron_bin( fileName::AbstractString )
"""
function load_neuron_bin( fileName::AbstractString )
    @assert endswith(fileName, ".neuron.bin")
    read( fileName ) |> deserialize
end 

function serialize(self::Neuron)
...
"""
    save_neuron_bin( self::Neuron, fileName::AbstractString )
represent swc file as binary file. the data structure is the same with swc.
"""
function save_neuron_bin( self::Neuron, fileName::AbstractString )
    @assert endswith(fileName, ".neuron.bin")
    data = serialize(self)
    write(fileName, data)
end 
...
"""
    downsample!(self::Neuron{T}; step::T = Float32(1000))
...
function downsample!(self::Neuron{T}; step::T = Float32(1000)) where T
    ...
    @fastmath @inbounds while !isempty(seedNodeIdxes)
        ...
        while childrenNum == 1
            ...
            if d2 < stepSquared
                ...
                # have enough walking distance, will include this node in new Neuron
    ...
    # pickout the selected nodes as new Neuron
    ...
    Neuron(newClasses, newNodeArray, newConnectivityMatrix)
end

@inline function add_offset!(self::Neuron{T}, offset::NTuple{3,T} ) where T
    add_offset!(self, [offset...])
end

function add_offset!(self::Neuron{T}, offset::Vector{T} ) where T
...
@inline function stretch_coordinates!(self::Neuron, mip::Real)
    expansion = [2^(mip-1), 2^(mip-1), 1]
    stretch_coordinates!(self, expansion)
end 

function stretch_coordinates!(self::Neuron{T}, expansion::Union{Vector, Tuple}) where T
